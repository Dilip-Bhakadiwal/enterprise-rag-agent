...
export const HeroContent: React.FC<HeroContentProps> = ({
  ...
  return (
    <div className="relative z-20 flex-1 flex flex-col justify-center px-5 sm:px-10 lg:px-16 w-full items-start text-left pt-20 sm:pt-24 pb-28 sm:pb-12 my-auto max-w-7xl">
      
      {/* 1. Tagline: Crown icon + tracked uppercase text (animate-fade-up, 0s delay) */}
      <div className="animate-fade-up flex items-center gap-2 mb-4 sm:mb-6 lg:mb-8">
      ...
      <div className="animate-fade-up-delay-3 mt-6 sm:mt-8 lg:mt-10 flex flex-wrap items-center gap-3 sm:gap-6 w-full">
        {/* Primary CTA: CHAT WITH RAG AGENT */}
        <button
          id="hero-rag-cta"
          onClick={() => onOpenRag()}
          className="group flex items-center gap-2 sm:gap-3 bg-black hover:bg-neutral-900 border border-white/20 hover:border-[#FF9F1C]/70 hover:shadow-[0_0_30px_rgba(255,159,28,0.3)] px-4 sm:px-7 py-3 sm:py-4 text-[10px] sm:text-xs font-inter tracking-widest uppercase text-white transition-all duration-300 cursor-pointer"
        >
          <span>Chat with RAG Agent</span>
          <ArrowUpRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200" />
        </button>

        {/* Secondary CTA: VIEW PROJECTS */}
        <button
          onClick={onOpenProjects}
          className="group flex items-center gap-2 border border-white/25 hover:border-white/60 hover:bg-white/10 px-4 sm:px-6 py-3 sm:py-4 text-[10px] sm:text-xs font-inter tracking-widest uppercase text-white transition-all duration-300 cursor-pointer"
        >
